...
parser.add_argument(
    ...
    type=int,
    default=-1,
    help='Alpaca split/shuffle seed: >=0 fixed (required for ablation); <0 uses timestamp',
)
parser.add_argument(
    '--mia_ablation_export',
    type=str,
    default='',
    help='If set: write JSON after training (mean/last dynamic threshold, ASR, roc_auc, etc.). Multi-window ablation: run_mia_online_window_ablation.py (FedMeZO-compatible .npz).',
)
parser.add_argument(
    '--mia_skip_summary_plots',
    action='store_true',
    default=False,
    help='Skip 2x2 summary plots after training (save time in batch ablations)',
)
...
if __name__ == "__main__":
    ...
    import time
    if int(getattr(args, "data_shuffle_seed", -1)) >= 0:
        data_shuffle_seed = int(args.data_shuffle_seed) % (2**31)
        logger.info(f"Data load fixed seed: {data_shuffle_seed} (--data_shuffle_seed)")
    else:
        data_shuffle_seed = int(time.time() * 1000000) % (2**31)
        logger.info(f"Data load random seed: {data_shuffle_seed} (timestamp)")
...
for round_idx in range(args.comm_round):
    ...
    if len(all_round_mia_stats['rounds']) > 0:
        ...
        exp_path = str(getattr(args, "mia_ablation_export", "") or "").strip()
        if exp_path:
            ms = getattr(trainer, "mia_stats", {}) or {}
            ablation_row = {
                "mia_online_window": int(getattr(args, "mia_online_window", 0)),
                "mia_online_update_with_neg_only": bool(int(getattr(args, "mia_online_update_with_neg_only", 1))),
                "mia_online_warmup": int(getattr(args, "mia_online_warmup", 0)),
                "mia_online_alpha": float(getattr(args, "mia_online_alpha", 0.1)),
                "mia_threshold_mode": str(getattr(args, "mia_threshold_mode", "")),
                "data_shuffle_seed": int(data_shuffle_seed),
                "mean_dynamic_threshold": float(ms.get("online_thr_mean", float("nan"))),
                "last_dynamic_threshold": float(ms.get("online_thr_last", float("nan"))),
                "online_thr_count": int(ms.get("online_thr_count", 0)),
                "asr": float(final_overall_acc),
                "roc_auc": float(ms.get("roc_auc", float("nan"))),
                "member_accuracy": float(final_member_acc),
                "non_member_accuracy": float(final_non_member_acc),
            }
            _exp_dir = os.path.dirname(os.path.abspath(exp_path))
            if _exp_dir:
                os.makedirs(_exp_dir, exist_ok=True)
            with open(exp_path, "w", encoding="utf-8") as ef:
                json.dump(ablation_row, ef, indent=2, ensure_ascii=False)
            logger.info("Ablation export written to: %s", exp_path)
        
        # Visualization (if possible)
        if getattr(args, "mia_skip_summary_plots", False):
            logger.info("Skipped 2x2 summary plots (mia_skip_summary_plots)")
        else:
            try:
                import matplotlib
                matplotlib.use('Agg')
