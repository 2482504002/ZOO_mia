...
# Prefer this repo (server-llama) code: federated multi-client changes live in local forward_training, avoiding stale trainers in FwdLLM-master
_REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
if os.path.isdir(os.path.join(_REPO_ROOT, "forward_training")):
    sys.path.insert(0, _REPO_ROOT)
# Remaining dependencies can still be resolved from FwdLLM-master
...
parser.add_argument('--fed_sim_num_clients', type=int, default=200,
                    help='Alpaca JSON: federated simulation client count (even train split); <=1 is single-machine full sequential training')
...
parser.add_argument('--alpaca_num_labels', type=int, default=4,
                    help='Alpaca JSON pseudo-label class count (only when loading Alpaca JSON)')
...
def _build_alpaca_dataloader(texts, labels, tokenizer, max_seq_length, batch_size, shuffle):
    ...
    # Compatible with BaseDataLoader: eval_model accesses test_dl.examples
    dataloader.examples = list(range(len(dataset)))
    dataloader.features = dataloader.examples
    return dataloader


def _even_chunk_ranges(n_samples, num_chunks):
    """Split [0, n) into num_chunks segments (first remainder segments get one extra sample)."""
    ranges = []
    base = n_samples // num_chunks
    rem = n_samples % num_chunks
    start = 0
...
if __name__ == "__main__":
    ...
    # 3. Load dataset attributes (Alpaca JSON uses custom branch to avoid h5 dependency)
    ...
    # 7. Create data preprocessor (JSON branch does not use this preprocessor)
    preprocessor = None
    ...
    alpaca_fed_client_loaders = None
    if use_alpaca_json:
        logger.info("Using Alpaca JSON loader (reference: distilbert-alpaca-MIA-zhibiao.py)")
        ...
        fed_n = int(getattr(args, 'fed_sim_num_clients', 1))
        if fed_n > 1:
            if fed_n > len(train_texts):
                logger.warning(
                    f"fed_sim_num_clients={fed_n} exceeds training samples {len(train_texts)}; clamping to sample count."
                )
            n_clients = min(fed_n, len(train_texts))
            if n_clients >= 2:
                chunk_spans = _even_chunk_ranges(len(train_texts), n_clients)
                alpaca_fed_client_loaders = [
                    _build_alpaca_dataloader(
                        train_texts[a:b],
                        train_labels[a:b],
                        tokenizer,
                        args.max_seq_length,
                ...
                logger.info(
                    f"Alpaca federated simulation clients={n_clients}, samples per client={[b - a for a, b in chunk_spans]}"
                )
    else:
    ...
    use_fed_alpaca = (
        alpaca_fed_client_loaders is not None and len(alpaca_fed_client_loaders) >= 2
    )
    total_train_samples_for_update = (
        sum(len(dl.dataset) for dl in alpaca_fed_client_loaders) if use_fed_alpaca else None
    )
    if use_fed_alpaca:
        logger.info(
            f"Alpaca federated simulation: each comm round trains {len(alpaca_fed_client_loaders)} clients sequentially and accumulates gradients "
            f"(total samples {total_train_samples_for_update}; change client count via --fed_sim_num_clients)"
        )
    ...
    for round_idx in range(args.comm_round):
        ...
        if use_fed_alpaca:
            for ci, c_dl in enumerate(alpaca_fed_client_loaders):
                trainer.train_dl = c_dl
                logger.info(f"  - Client {ci + 1}/{len(alpaca_fed_client_loaders)}, local samples={len(c_dl.dataset)}")
                global_step, tr_loss = trainer.train_model(device, fed_segment_index=ci)
            trainer.train_dl = train_dl
        else:
        ...
        if hasattr(trainer, 'apply_gradient_update'):
            trainer.apply_gradient_update(
                current_round=round_idx,
                training_num_override=total_train_samples_for_update,
            )
