...
# Prefer the current server-llama repo to avoid loading the old Trainer from FwdLLM-master.
...
parser.add_argument(
    '--data_file_path',
    type=str,
    default='/home/zhike/JWH/data/agnews/data',
    help='Local AGNEWS Parquet directory (Llama custom loader branch)',
)
...
parser.add_argument(
    '--bbc_aux_data_path',
    type=str,
    default='/home/zhike/JWH/data/bbc-news/bbc-text.csv',
    help='External MIA auxiliary dataset; targets and client train data still from AGNEWS',
)
...
parser.add_argument('--mia_aux_samples_fixed', type=int, default=0,
...
parser.add_argument('--mia_aux_agnews_divisor', type=int, default=9,
                    help='AGNews without fixed aux: num_aux=max(1, total_samples // divisor); DistilBERT default 1000')
...
class AGNewsTupleDataset(Dataset):
...
def _build_agnews_dataloader(texts, labels, tokenizer, max_seq_length, batch_size, shuffle):
    encoded = tokenizer(
        texts,
        truncation=True,
        padding="max_length",
    ...
    dataset = AGNewsTupleDataset(input_ids, attention_mask, label_tensor)
...
def _load_agnews_texts_labels(total_needed, seed, data_dir):
    ...
    if not os.path.isfile(train_path):
        raise FileNotFoundError(
            f"Local AGNEWS train parquet missing: {train_path}; "
            "No auto-download; set --data_file_path to the correct directory."
        )

    import pandas as pd

    frame = pd.read_parquet(train_path, columns=["text", "label"])
    n = min(int(total_needed), len(frame))
    texts = frame["text"].iloc[:n].astype(str).tolist()
    labels = frame["label"].iloc[:n].astype(int).tolist()
    logger.info(
        "Loaded AGNEWS from local parquet: %s (available=%d selected=%d)",
        train_path,
        len(frame),
        n,
    )
...
def _load_bbc_auxiliary_pool(csv_path, tokenizer, max_seq_length):
    """
    Convert BBC News into Trainer-compatible single-sample (input_ids, AGNEWS label) list.

    BBC -> AGNEWS semantic labels:
      politics -> World(0), sport -> Sports(1),
      business -> Business(2), tech -> Sci/Tech(3).
    BBC entertainment has no AGNEWS class; excluded to avoid wrong supervision polluting JVP.
    """
    label_map = {
        "politics": 0,
        "sport": 1,
    ...
    if not os.path.isfile(csv_path):
        raise FileNotFoundError(f"BBC auxiliary dataset not found: {csv_path}")
    ...
    with open(csv_path, "r", encoding="utf-8", newline="") as f:
        ...
        if not reader.fieldnames or "category" not in reader.fieldnames or "text" not in reader.fieldnames:
            raise ValueError(
                f"BBC CSV must contain category,text columns; got {reader.fieldnames}"
            )
        for row in reader:
            category = (row.get("category") or "").strip().lower()
            text = (row.get("text") or "").strip()
            if category not in label_map:
                skipped_categories[category or "<empty>"] = (
                    skipped_categories.get(category or "<empty>", 0) + 1
                )
                continue
            if not text:
                continue
    ...
    if not texts:
        raise ValueError(f"No usable BBC auxiliary samples found in {csv_path}")
    ...
    label_tensor = torch.tensor(labels, dtype=torch.long)
    pool = [
        (input_ids[i : i + 1].clone(), label_tensor[i : i + 1].clone())
    ...
    logger.info(
        "[BBC auxiliary] source=%s usable=%d AGNEWS_label_distribution=%s skipped=%s",
        csv_path,
        len(pool),
        dict(sorted(counts.items())),
        dict(sorted(skipped_categories.items())),
    )
    return pool
...
if __name__ == "__main__":
    ...
    # 3. Load dataset attributes (Llama + agnews custom branch, bypassing TLMPreprocessor tokenizer incompatibility)
    use_custom_agnews_loader = (args.model_type == "llama" and args.dataset.lower() == "agnews")
    logger.info(f"Loading dataset from: {args.data_file_path}")
    if use_custom_agnews_loader:
        num_labels = 4
        attributes = {"label_vocab": list(range(num_labels))}
        logger.info(f"Dataset: {args.dataset} (custom loader), Labels: {num_labels}")
    ...
    model_args.update_from_dict({
        ...
        "data_file_path": args.data_file_path,
                                ...
                                "bbc_aux_data_path": args.bbc_aux_data_path,
    ...
    # 7. Create data preprocessor (not needed for custom branch)
    preprocessor = None
    if not use_custom_agnews_loader:
    ...
    # Re-randomize before data load so order differs each run
    ...
    if use_custom_agnews_loader:
        logger.info("Using custom AGNews loader for Llama tokenizer compatibility...")
        total_needed = max(
        ...
        if len(all_texts) < 2:
            raise ValueError(f"AGNews too few samples: {len(all_texts)}, need at least 2.")
    ...
    if (not use_custom_agnews_loader) and hasattr(train_dl, 'dataset') and train_dl.dataset is not None:
    ...
    if (not use_custom_agnews_loader) and hasattr(full_train_dl, 'dataset') and full_train_dl.dataset is not None:
    ...
    if (not use_custom_agnews_loader) and args.max_train_samples is not None:
    ...
    if (not use_custom_agnews_loader) and args.max_test_samples is not None:
    ...
    # AGNEWS still used for client train, test, and target sample selection.
    trainer.set_data(train_dl, test_dl, full_train_dl=full_train_dl)

    # MIA auxiliary samples from same-domain, different-source BBC News.
    # select_mia_attack_bundle picks AGNEWS targets from trainer.train_dl,
    # but samples auxiliary only from this external pool.
    bbc_aux_pool = _load_bbc_auxiliary_pool(
        args.bbc_aux_data_path,
        tokenizer,
        args.max_seq_length,
    )
    model_args.mia_aux_external_pool = bbc_aux_pool
    from forward_training.server_mia_initialization import select_mia_attack_bundle

    trainer.mia_attack_bundle = select_mia_attack_bundle(
        trainer,
        mia_aux_seed=int(args.manual_seed),
    )
    trainer.mia_attack_bundle["auxiliary_source"] = "bbc-news"
    trainer.mia_attack_bundle["auxiliary_data_path"] = args.bbc_aux_data_path
    logger.info(
        "[MIA cross-source] target_source=AGNEWS auxiliary_source=BBC-News "
        "target_label=%d auxiliary_samples=%d",
        int(trainer.mia_attack_bundle["target_sample"][1].item()),
        len(trainer.mia_attack_bundle["aux_samples"]),
    )
    ...
    logger.info(f"Full AGNEWS samples: {len(full_train_dl.dataset)} (target pool)")
    logger.info(f"BBC auxiliary pool samples: {len(bbc_aux_pool)}")
...
for round_idx in range(args.comm_round):
    ...
    # Collect MIA stats if present
    if hasattr(trainer, 'mia_stats') and trainer.mia_stats['member_total'] > 0:
        ...
        # Apply gradient update (FedSgdAggregator-style)
        ...
        if (round_idx + 1) % args.frequency_of_the_test == 0:
            ...
            metrics, _, _ = trainer.eval_model()  # returns (result, model_outputs, wrong)
    ...
    if len(all_round_mia_stats['rounds']) > 0:
        ...
        report_data = {
            'attack_config': {
                ...
                'auxiliary_dataset': 'BBC-News',
                'auxiliary_data_path': args.bbc_aux_data_path,
                'auxiliary_pool_size': len(bbc_aux_pool),
                'auxiliary_samples_used': len(trainer.mia_attack_bundle['aux_samples']),
                'bbc_to_agnews_label_map': {
                    'politics': 0,
                    'sport': 1,
                    'business': 2,
                    'tech': 3,
